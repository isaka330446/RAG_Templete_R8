...
def expected_source_candidates_from_json(value: Any) -> str:
    ...
    for source in value if isinstance(value, list) else [value]:
        ...
        for key in ("heading", "evidence_quote", "source_url", "taxanswer_no", "document_id"):
...
def summarize_predictions(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for mode in sorted({str(row.get("retrieval_mode") or "unknown") for row in ok_rows}):
        mode_rows = [row for row in ok_rows if str(row.get("retrieval_mode") or "unknown") == mode]
        mode_hits = sum(1 for row in mode_rows if row.get("expected_source_hit") is True or str(row.get("expected_source_hit")).lower() == "true")
        mode_latencies = [float(row.get("elapsed_sec") or 0.0) for row in mode_rows]
        by_mode[mode] = {
            "ok": len(mode_rows),
            "expected_source_hit_rate": mode_hits / len(mode_rows) if mode_rows else 0.0,
            "avg_elapsed_sec": sum(mode_latencies) / len(mode_latencies) if mode_latencies else 0.0,
        }
    return {
        "total": total,
        "ok": len(ok_rows),
