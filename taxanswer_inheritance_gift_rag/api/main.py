...
def _optional_int_query(name: str, value: str | None) -> int | None:
...
def _optional_score_query(name: str, value: str | None) -> float | None:
    text = str(value or "").strip()
    if not text:
        return None
    try:
...
def admin_search_logs(
    ...
    qa_cache_id: str | None = Query(default=None),
    session_id: str | None = Query(default=None),
    log_id: str | None = Query(default=None),
    min_score: str | None = Query(default=None),
    max_score: str | None = Query(default=None),
):
    qa_cache_id_value = _optional_int_query("qa_cache_id", qa_cache_id)
    log_id_value = _optional_int_query("log_id", log_id)
    min_score_value = _optional_score_query("min_score", min_score)
    max_score_value = _optional_score_query("max_score", max_score)
    return {
        "logs": engine.log_store.search(
            ...
            qa_cache_id=qa_cache_id_value,
            session_id=session_id,
            log_id=log_id_value,
            min_score=min_score_value,
            max_score_filter=max_score_value,
        ),
        "logging": engine.log_store.status(),
    }
...
@app.post("/admin/evidence/search/vector")
def admin_evidence_search_vector(req: EvidenceSearchRequest):
    corpus_ids = resolve_request_corpus_ids(req.corpus_ids)
    sources = engine.retriever.search_vector_only(req.query, corpus_ids=corpus_ids, top_k=req.top_k)
    return {
        "mode": "vector",
        "query": req.query,
        "top_k": req.top_k,
...
@app.post("/admin/evidence/search/hybrid")
def admin_evidence_search_hybrid(req: EvidenceSearchRequest):
    corpus_ids = resolve_request_corpus_ids(req.corpus_ids)
    sources = engine.retriever.search_hybrid_no_reranker(req.query, corpus_ids=corpus_ids, top_k=req.top_k)
    return {
        "mode": "hybrid",
        "query": req.query,
        "top_k": req.top_k,
...
@app.post("/admin/evidence/search/compare")
def admin_evidence_search_compare(req: EvidenceSearchRequest):
    corpus_ids = resolve_request_corpus_ids(req.corpus_ids)
    vector_sources = engine.retriever.search_vector_only(req.query, corpus_ids=corpus_ids, top_k=req.top_k)
    hybrid_sources = engine.retriever.search_hybrid_no_reranker(req.query, corpus_ids=corpus_ids, top_k=req.top_k)
    reranker_sources = engine.retriever.search(req.query, corpus_ids=corpus_ids, top_k=req.top_k)
    return {
        "query": req.query,
        "top_k": req.top_k,
        "corpus_ids": corpus_ids,
        "reranker_enabled": getattr(engine.retriever, "reranker_enabled", False),
        "modes": {
            "vector": {"sources": vector_sources},
            "hybrid": {"sources": hybrid_sources},
            "hybrid_reranker": {"sources": reranker_sources},
        },
    }


@app.post("/admin/evidence/answer/vector")
def admin_evidence_answer_vector(req: EvidenceSearchRequest):
    corpus_ids = resolve_request_corpus_ids(req.corpus_ids)
    return engine.answer_with_retrieval_mode(
        req.query,
        "vector",
        corpus_ids=corpus_ids,
        top_k=req.top_k,
...
@app.post("/admin/evidence/answer/hybrid")
def admin_evidence_answer_hybrid(req: EvidenceSearchRequest):
    corpus_ids = resolve_request_corpus_ids(req.corpus_ids)
    return engine.answer_with_retrieval_mode(
        req.query,
        "hybrid",
...
@app.post("/admin/evidence/answer/hybrid_reranker")
def admin_evidence_answer_hybrid_reranker(req: EvidenceSearchRequest):
    corpus_ids = resolve_request_corpus_ids(req.corpus_ids)
    return engine.answer_with_retrieval_mode(
        req.query,
        "hybrid_reranker",
