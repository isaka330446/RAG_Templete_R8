# 本番向けRAG API、管理者用API、リリース管理APIをFastAPIで公開します。
import csv
import json
...
def create_approved_qa(req: ApprovedQARequest):
    try:
        ...
        qa_id = engine.answer_cache.create_approved_qa(
            ...
            corpus_version=req.corpus_version or engine.retriever.corpus_version,
            index_version=req.index_version or engine.retriever.index_version,
...
def admin_releases():
    manager = engine.retriever.release_manager
    return {
...
def admin_activate_release(release_id: str):
    try:
        release = engine.retriever.release_manager.activate_release(release_id)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    active = engine.reload_retriever()
    return {
        "release": release,
        "active": active,
    }
