# RAG 標準テンプレート（親子RAG + SearchTag版）

このテンプレートは、ローカルLLM / vLLM / ChromaDB / bge-m3 を前提にした、会社導入向けの標準RAG構成です。
親子RAGとSearchTagまでを標準装備にし、ハイブリッド検索やrerankerは `../rag_template_hybrid_reranker/` に分けています。
...
- API/LLM/Embedding/検索/CORSは `config/settings.json` と環境変数から読み込まれます。環境変数がある場合は環境変数を優先します。
...
- BM25とrerankerは標準版には含めません。必要な場合は `../rag_template_hybrid_reranker/` を使います。
