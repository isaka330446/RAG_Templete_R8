...
class HybridRetriever:
    ...
    def _result_row(self, child: dict, score: float, auxiliary_reason: str | None = None) -> dict:
        ...
        if auxiliary_reason:
            row["auxiliary_context"] = True
            row["auxiliary_reason"] = auxiliary_reason
        return row

    def _related_meeting_chunks(
        self,
        seed_rows: List[dict],
        allowed: Optional[set],
        limit: int = 4,
    ) -> List[dict]:
        seed_rows = [row for row in seed_rows if row.get("meeting_id")]
        if not seed_rows:
            return []

        seen_child_ids = {row.get("child_id") for row in seed_rows}
        meeting_ids = {str(row.get("meeting_id")) for row in seed_rows if row.get("meeting_id")}
        agendas = {str(row.get("agenda")) for row in seed_rows if row.get("agenda")}
        topics = {str(row.get("topic")) for row in seed_rows if row.get("topic")}
        source_types = {str(row.get("source_type")) for row in seed_rows if row.get("source_type")}

        candidates = []
        for child in self.children:
            if child.get("child_id") in seen_child_ids:
                continue
            if allowed is not None and child.get("corpus_id") not in allowed:
                continue
            if str(child.get("meeting_id") or "") not in meeting_ids:
                continue

            score = 0.10
            reasons = ["same_meeting_id"]
            if child.get("agenda") and str(child.get("agenda")) in agendas:
                score += 0.25
                reasons.append("same_agenda")
            if child.get("topic") and str(child.get("topic")) in topics:
                score += 0.25
                reasons.append("same_topic")
            if child.get("source_type") and str(child.get("source_type")) not in source_types:
                score += 0.20
                reasons.append("different_source_type")
            candidates.append((score, child, ",".join(reasons)))

        candidates.sort(key=lambda item: item[0], reverse=True)
        return [
            self._result_row(child, score=score, auxiliary_reason=reason)
            for score, child, reason in candidates[:limit]
        ]
    ...
    def search(self, query: str, corpus_ids: Optional[List[str]] = None, top_k: Optional[int] = None) -> List[dict]:
        ...
        for cid in merged_ids:
            ...
            rows.append(self._result_row(child, score=score))
        ...
        related = self._related_meeting_chunks(final, allowed, limit=4)
        return final + related
