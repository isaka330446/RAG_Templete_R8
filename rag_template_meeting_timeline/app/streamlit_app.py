...
answer_mode = st.sidebar.selectbox(
    "回答モード",
    options=["auto", "rag", "timeline"],
    index=0,
    format_func=lambda value: {"auto": "自動", "rag": "通常RAG", "timeline": "時系列"}[value],
)
with st.sidebar.expander("時系列フィルタ", expanded=False):
    event_type_filter = st.selectbox(
        "イベント種別",
        options=["", "decision", "action_item", "concern", "pending", "rejection", "proposal", "discussion", "change", "report", "completion"],
    )
    source_type_filter = st.selectbox("根拠種別", options=["", "slide", "minutes"])
    date_from_filter = st.text_input("開始日 YYYY-MM-DD", value="")
    date_to_filter = st.text_input("終了日 YYYY-MM-DD", value="")
    owner_filter = st.text_input("担当者", value="")
    status_filter = st.text_input("状態", value="")
