...
def page_css() -> str:
    ...
    * { box-sizing:border-box; }
    ...
    .admin-modal-backdrop { position:absolute; inset:0; background:rgba(15,23,42,.42); }
    .admin-modal-card {
      position:relative;
    ...
    .admin-modal-head { flex:0 0 auto; padding:18px 20px; border-bottom:1px solid var(--line); display:flex; align-items:center; justify-content:space-between; gap:12px; }
    .admin-modal-body { overflow:auto; height:100%; padding:18px; }
    ...
    @media (max-width:1180px) {
      ...
      .qa-workspace-grid { grid-template-columns:1fr; }
...
async def reports_content(base_url: str, request: Request) -> str:
    ...
    detail_html = report_detail_html(base_url, detail) if detail else '<div class="panel"><h2>報告詳細</h2><div class="empty">未対応報告はありません。ユーザーから報告が入るとここに表示されます。</div></div>'
    return f"""
    <section class="panel">
      <h2>報告対応</h2>
    ...
    <section class="panel">
      <div class="panel"><h2>報告一覧</h2>{data_table(["ID","状態","日時","質問","ユーザーコメント","操作"], rows, "この条件の報告はありません。")}</div>
      {report_detail_modal(base_url, detail, {'status': status}) if detail else ""}
    </section>
    """
...
def report_detail_modal(base_url: str, detail: dict, close_params: dict | None = None) -> str:
    ...
    return f"""
    <div class="admin-modal" role="dialog" aria-modal="true" aria-label="Report detail">
      <a class="admin-modal-backdrop" href="{close_href}" aria-label="Close"></a>
      <section class="admin-modal-card log-detail-modal">
        <header class="admin-modal-head">
          <div>
            <h2>Report #{esc(report.get("id"))}</h2>
            <p class="lead">Confirm the report, root-cause memo, response status, and QA registration in a wide modal.</p>
          </div>
          <a class="modal-close" href="{close_href}">Close</a>
        </header>
...
async def qa_content(base_url: str, request: Request) -> str:
    q = request.query_params
    status = q.get("status") or "all"
    keyword = q.get("q") or ""
    qa_id = q.get("qa_id")
    draft_question = q.get("question") or keyword
    try:
    ...
    similar_html = await qa_similar_panel(base_url, draft_question) if draft_question else '<div class="empty">質問を入力すると類似QAと現在のマッチ結果を確認できます。</div>'
    ...
    <section class="panel">
      <h2>QA候補作成・類似確認</h2>
      <p class="lead">質問を起点に、既存QAとの近さと根拠候補を同じ場所で確認します。</p>
      <div class="qa-workspace-grid">
        <div class="qa-workspace-main">
          <form method="get" action="/qa-cache" data-loading-message="類似QAと根拠候補を確認しています...">
            <div class="field"><label>質問</label><textarea name="question">{esc(draft_question)}</textarea></div>
            <div class="right-actions">
              <button class="btn" type="submit">類似QAと根拠候補を確認</button>
            </div>
          </form>
    ...
    <section class="panel">
      ...
      <form method="post" action="/qa-register" data-loading-message="承認QAを登録しています...">
        <div class="field"><label>質問</label><textarea name="question">{esc(draft_question)}</textarea></div>
        <div class="field"><label>回答</label><textarea name="answer"></textarea></div>
        <div class="notice">根拠は1件以上選ぶ運用を推奨します。どうしても根拠なしで登録する場合は、管理メモに理由を残してください。</div>
        <details open><summary>選択済み根拠JSON</summary><textarea id="qa_evidence_json" name="evidence_json">[]</textarea></details>
        <div class="field"><label>承認者</label><input name="approved_by" value="admin"></div>
      ...
      <section class="admin-modal-card">
        <header class="admin-modal-head">
          <div>
            ...
            <p class="lead">質問・回答・根拠・aliasをまとめて編集します。</p>
          </div>
          <a class="modal-close" href="{with_base('/qa-cache', base_url)}">閉じる</a>
        </header>
        <div class="admin-modal-body">
          <div class="admin-modal-grid">
            <div>{qa_detail_panel(base_url, item)}</div>
            <div>{await qa_alias_panel(base_url, item)}</div>
          </div>
        </div>
      </section>
    </div>
...
async def qa_alias_panel(base_url: str, item: dict) -> str:
    ...
    <div class="panel">
      <h2>Alias管理</h2>
      <p class="lead">approved_qaは回答本体、aliasは検索入口です。対象文書違い・条件追加・回答範囲が広がるaliasは無効化してください。</p>
      ...
      <div class="grid-2" style="margin-top:14px">
        <form method="post" action="/qa-alias-generate">
          <input type="hidden" name="qa_id" value="{qa_id}">
      ...
      <div class="grid-2" style="margin-top:14px">
        ...
        <form method="post" action="/qa-cache-reload-index">
...
def qa_detail_panel(base_url: str, item: dict) -> str:
    ...
    <div class="panel">
      ...
      <form method="post" action="/qa-update">
        ...
        <div class="field"><label>回答</label><textarea name="answer">{esc(item.get("answer"))}</textarea></div>
...
def log_detail_panel(row: dict | None) -> str:
    if not row:
        return ""
    return f"""
    <section class="panel">
...
async def handle_qa_register(request: Request) -> RedirectResponse:
    form = await request.form()
    base_url = normalize_base_url(form.get("base_url"))
...
async def handle_qa_alias_generate(request: Request) -> HTMLResponse | RedirectResponse:
    ...
    <section class="panel">
      ...
      <div class="grid-2" style="margin-top:14px">
        <form method="post" action="/qa-alias-add">
          <input type="hidden" name="qa_id" value="{qa_id}">
