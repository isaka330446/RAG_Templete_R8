...
def load_predictions(path: Path, limit: int = 0, include_no_answer: bool = False) -> list[dict[str, Any]]:
...
def main() -> None:
    ...
    rows = load_predictions(input_path, args.limit, include_no_answer=args.include_no_answer)
    records = []
    for row in rows:
