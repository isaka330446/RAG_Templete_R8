    ...
    def _normalize_cache_sources(self, evidence: List[dict]) -> List[dict]:
        ...
        for idx, item in enumerate(evidence or [], start=1):
            ...
            row = {
                ...
                "document_series": item.get("document_series"),
    ...
    def _build_contexts_from_sources(self, sources: List[dict]) -> List[dict]:
        ...
        for s in sources:
            ...
            contexts.append({
                ...
                "document_series": s.get("document_series"),
                "document_code": s.get("document_code"),
...
対象文書違い、条件追加、回答範囲拡大、個別判断が必要な場合はfalseです。
JSONのみで返してください。
...
"""
        try:
            text = self.alias_llm.chat([
                {"role": "system", "content": "あなたはRAGキャッシュの安全判定を行います。JSONだけを返してください。"},
