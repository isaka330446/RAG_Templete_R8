...
def normalize_qa_text(text: str) -> str:
    ...
    value = re.sub(r"\bdoc\s*\.?\s*([0-9a-z_-]+)", r"doc\1", value)
    value = re.sub(r"文書\s*id\s*[:：]?\s*([0-9a-z_-]+)", r"文書id\1", value, flags=re.IGNORECASE)
    value = re.sub(r"[ \t\r\n]+", " ", value)
...
def extract_document_ids(text: str) -> set[str]:
    ...
    patterns = [
        r"(?:document|doc|??)\s*(?:id)?\s*[:?#-]?\s*([A-Za-z0-9][A-Za-z0-9_-]{2,40})",
        r"\b([A-Z]{2,}[-_]?[0-9]{2,})\b",
    ]
    found: set[str] = set()
    for pattern in patterns:
        found.update(m.group(1).casefold() for m in re.finditer(pattern, value, flags=re.IGNORECASE))
    return found
...
def detect_alias_risk_flags(question: str, alias_text: str, evidence: list[dict[str, Any]] | None = None) -> list[str]:
    ...
    source_ids = extract_document_ids(source)
    for item in evidence or []:
        if isinstance(item, dict):
            source_ids.update(extract_document_ids(item.get("document_id") or ""))
            source_ids.update(extract_document_ids(item.get("title") or ""))
    alias_ids = extract_document_ids(alias)
    if source_ids and alias_ids and source_ids.isdisjoint(alias_ids):
        flags.append("document_id_changed")

    condition_words = ["個別", "具体", "ケース", "判断", "例外", "条件", "年度", "改定", "改正", "最新版"]
    if any(word in alias for word in condition_words) and not any(word in source for word in condition_words):
        flags.append("scope_or_time_condition_added")

    broad_words = ["すべて", "全般", "全体", "一覧", "網羅", "どんな場合"]
    if any(word in alias for word in broad_words) and not any(word in source for word in broad_words):
        flags.append("answer_scope_expanded")
    return flags
...
def build_alias_generation_prompt(question: str, answer: str, evidence: list[dict[str, Any]], max_aliases: int) -> str:
    return f"""あなたはRAGの承認済みQAキャッシュを管理する補助者です。
    ...
    def match_debug(
        ...
        *,
        ...
        for candidate in candidates:
            candidate["would_hit"] = (
                float(candidate.get("similarity") or 0.0) >= accept_threshold
                and (candidate is not candidates[0] or margin >= self.margin_threshold)
            )
    ...
    def backfill_aliases(
        ...
        *,
        ...
        with self._connect() as conn:
            ...
            missing_original_count = conn.execute(
                """
                SELECT COUNT(*)
                FROM approved_qa qa
                WHERE qa.status = 'approved'
                  AND qa.corpus_version = ?
                  AND qa.index_version = ?
                  AND NOT EXISTS (
                      SELECT 1 FROM approved_qa_aliases a
                      WHERE a.qa_id = qa.id AND a.alias_type = 'original' AND a.status = 'active'
                  )
            ...
            original_result = {"checked": 0, "created": 0, "skipped": 0}
            if ensure_original and not dry_run:
                original_result = self._ensure_original_aliases_conn(conn)
        if not dry_run:
            self.reload_alias_index()
        return {
            "enabled": True,
